...
export function fInitMcsv_bridge() {
  ...
  function fRestoreState() {
    try {
      var a = sessionStorage.getItem('mcsvAnchor'), sSc = sessionStorage.getItem('mcsvScroll'), bRestored = false;
      if (a) {
        var p = a.split('|');
        if (Date.now() - (+p[2]) < 15000) {
          var oEl = document.querySelector('.clsMcsvT[data-id="' + p[0] + '"][data-ord="' + p[1] + '"]');
          if (oEl) { oEl.scrollIntoView({ block: 'center' }); fPlaceCaretEnd(oEl); oEl.focus({ preventScroll: true }); bRestored = true; }
        }
        sessionStorage.removeItem('mcsvAnchor');
      }
      // Don't clobber an in-page #hash jump (a fresh nav to file#name, which mMcsh2
      // scrolls to on load): restore the saved scroll only when there's no edit-line
      // anchor AND no hash to honor. The structural-reload anchor path above still wins.
      if (!bRestored && sSc != null && !location.hash) window.scrollTo(0, parseInt(sSc, 10) || 0);
    } catch (e) {}
  }
  var nScrollT;
  ...
  function fDoAnnotate(ids) {
    ...
    fRestoreState();                // land back on the line after a structural reload
    setTimeout(fRestoreState, 60);  // beat mMcsh2's own location.hash scroll
  }
  window.addEventListener('message', function (e) {
    ...
    // cache-busts so freshly-saved bytes show up.
      ...
      fSaveState(document.querySelector('.clsMcsvT:focus') || oLastSpan); // keep the edit line across the save-reload
